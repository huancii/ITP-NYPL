...
def start_test(self, collection, model, caption, b):
    ...
    for im in tqdm(caption.images.all()):
        ...
        if not im.original_confidence_level:
...
@task(bind=True)
def update_image(self, collection, uuid, user):
    page = 0
    captures = []
    error_uris = []
    error_count = 0
    ...
    captures = captures[len(collection.images.all()):len(df)]
    quality = "w"
    threadnum = 5
    imgslimit = None  # maximum images
...
@task(bind=True)
def start_upload_test(self, caption, model):
    sol, lf, hw = init_model(model)
    sol.cuda()
    lf.cuda()
    hw.cuda()
    ...
    recognize_results = dict()
    all_length = len(caption.images.all())
    for im in tqdm(caption.images.all()):
        begin_time = time.time()
        image = im.image
        org_img = cv2.imread(image.path)
        ...
        end_time = time.time()
        run_time = end_time - begin_time
        caption.estimated_time = (run_time / 60 / 60) * all_length
        caption.save()
        all_length -= 1
    caption.estimated_time = 0
    caption.save()
    return recognize_results
